'use strict';

const API = ''; // Set to 'http://localhost:8080' if running from file://

/* ── Color maps ─────────────────────────────────────────── */
const TYPE_COLORS = {
  BIRTHDAY: { bar: '#F59E0B', bg: '#FEF3C7', text: '#92400E', label: 'Birthday' },
  FAMILY:   { bar: '#10B981', bg: '#D1FAE5', text: '#065F46', label: 'Family' },
  FRIENDS:  { bar: '#3B82F6', bg: '#DBEAFE', text: '#1E40AF', label: 'Friends' },
  MEETING:  { bar: '#8B5CF6', bg: '#EDE9FE', text: '#5B21B6', label: 'Meeting' },
};

const STATUS_STYLES = {
  FREE:     { fill: '#F9FAFB', stroke: '#E5E7EB', w: 1.5 }, // Neutru
  PARTIAL:  { fill: '#FEF3C7', stroke: '#F59E0B', w: 1.5 },
  OCCUPIED: { fill: '#FEF3C7', stroke: '#F59E0B', w: 1.5 },
};

const TABLE_COLORS = {
  1: { bar:'#A9DFBF', bg:'#E9F7EF', text:'#1D8348', label:'Green' },   // Masa 1: Verde (Green)
  2: { bar:'#D7BDE2', bg:'#F5EEF8', text:'#76448A', label:'Lilac' },  // Masa 2: Lila (Lilac)
  3: { bar:'#F9E79F', bg:'#FEF9E7', text:'#9A7D0A', label:'Yellow' }, // Masa 3: Galben (Yellow)
  4: { bar:'#F5CBA7', bg:'#FDF2E9', text:'#A04000', label:'Orange' }, // Masa 4: Portocaliu (Orange)
  5: { bar:'#AED6F1', bg:'#EBF5FB', text:'#1B4F72', label:'Blue' },   // Masa 5: Albastru (Blue)
  6: { bar:'#F5B7B1', bg:'#FDF1F3', text:'#943126', label:'Rose' },   // Masa 6: Roz (Rose)
  7: { bar:'#A3E4D7', bg:'#E8F8F5', text:'#148F77', label:'Teal' },   // Masa 7: Mint (Teal)
};

const ACCENT = '#C2643A';

/* ── Table layout config (center x, center y, seats) ──── */
const TABLE_LAYOUT = [
  { id: 1, cx: 140, cy: 160, seats: 3, zone: 'INDOOR' },
  { id: 2, cx: 310, cy: 160, seats: 2, zone: 'INDOOR' },
  { id: 3, cx: 140, cy: 310, seats: 4, zone: 'INDOOR' },
  { id: 4, cx: 310, cy: 310, seats: 4, zone: 'INDOOR' },
  { id: 5, cx: 580, cy: 160, seats: 4, zone: 'OUTDOOR' },
  { id: 6, cx: 750, cy: 160, seats: 5, zone: 'OUTDOOR' },
  { id: 7, cx: 665, cy: 310, seats: 6, zone: 'OUTDOOR' },
];

const DAYS = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
const MONTHS = ['January','February','March','April','May','June',
  'July','August','September','October','November','December'];

/* ── State ──────────────────────────────────────────────── */
let currentDate   = new Date();
let selectedTable = null;
let tablesData    = [];
let reservations  = []; // All reservations for current filtered date (or all)
let currentView   = 'dashboard';
let pendingTableId = null;

/* ── DOM ────────────────────────────────────────────────── */
const $ = id => document.getElementById(id);

/* ── Init ───────────────────────────────────────────────── */
(function init() {
  renderDateTitle();
  loadData();
  
  $('datePrev').onclick  = () => { currentDate.setDate(currentDate.getDate()-1); handleDateChange(); };
  $('dateNext').onclick  = () => { currentDate.setDate(currentDate.getDate()+1); handleDateChange(); };
  $('dateToday').onclick = () => { currentDate = new Date(); handleDateChange(); };
  
  $('detailClose').onclick = closePanel;
  $('btnCancel').onclick   = closePanel;
  
  // Navigation
  $('nav-dashboard').onclick    = () => switchView('dashboard');
  $('nav-reservations').onclick = () => switchView('reservations');
  $('nav-tables').onclick       = () => switchView('tables');
  $('nav-clients').onclick      = () => switchView('clients');

  // Escape key to close panel
  window.addEventListener('keydown', e => {
    if (e.key === 'Escape') closePanel();
  });
})();

function handleDateChange() {
  renderDateTitle();
  loadData();
}

function switchView(view) {
  currentView = view;
  document.querySelectorAll('.nav-item').forEach(n => {
    n.classList.toggle('active', n.id === `nav-${view}`);
  });
  closePanel();
  render();
}

/* ── Data ───────────────────────────────────────────────── */
async function loadData() {
  try {
    const [tRes, rRes] = await Promise.all([
      fetch(`${API}/api/tables?datetime=${encodeURIComponent(fmtISO(new Date(), new Date().getHours(), 0))}`),
      fetch(`${API}/api/reservations`),
    ]);
    tablesData = await tRes.json();
    const allRes = await rRes.json();
    
    // Filter reservations for current day
    const curDateStr = fmtISO(currentDate, 0, 0).split('T')[0];
    reservations = allRes.filter(r => r.dateTime.startsWith(curDateStr));
    
    // Cache all reservations in state if needed for clients (we use allRes for clients)
    window.allReservations = allRes;
  } catch { 
    tablesData = []; 
    reservations = []; 
    window.allReservations = [];
  }
  render();
  renderResListSidebar();
}

function render() {
  const container = document.querySelector('.main-scroll');
  const timeRuler = $('timeRuler');
  const floorPlan = $('floorPlan');
  
  if (currentView === 'dashboard') {
    timeRuler.style.display = 'block';
    floorPlan.style.display = 'flex';
    renderTimeRuler();
    renderFloorPlan();
    const dyn = container.querySelector('.dynamic-content');
    if (dyn) dyn.remove();
  } else {
    timeRuler.style.display = 'none';
    floorPlan.style.display = 'none';
    
    let dyn = container.querySelector('.dynamic-content');
    if (!dyn) {
      dyn = document.createElement('div');
      dyn.className = 'dynamic-content view-container';
      container.appendChild(dyn);
    }
    
    if (currentView === 'reservations') renderReservations(dyn);
    else if (currentView === 'tables') renderTables(dyn);
    else if (currentView === 'clients') renderClients(dyn);
  }
}

function renderDateTitle() {
  const d = currentDate;
  $('dateTitle').textContent =
    `${DAYS[d.getDay()]}, ${d.getDate()} ${MONTHS[d.getMonth()]} ${d.getFullYear()}`;
}

function renderTimeRuler() {
  const el = $('timeRuler');
  el.innerHTML = '';
  for (let h = 10; h <= 23; h++) {
    const pct = ((h - 10) / 13) * 100;
    const tick = document.createElement('div');
    tick.className = 'time-tick';
    tick.style.left = pct + '%';
    tick.textContent = String(h).padStart(2,'0') + ':00';
    el.appendChild(tick);
  }
  const now = new Date();
  const nowH = now.getHours() + now.getMinutes() / 60;
  if (nowH >= 10 && nowH <= 23) {
    const pct = ((nowH - 10) / 13) * 100;
    const line = document.createElement('div');
    line.className = 'time-now-line';
    line.style.left = pct + '%';
    line.innerHTML =
      `<div class="time-now-dot"></div>` +
      `<div class="time-now-label">${pad2(now.getHours())}:${pad2(now.getMinutes())}</div>`;
    el.appendChild(line);
  }
}

/* ── Floor Plan SVG Architectural tables ─────────────────── */

function svgEl(tag, attrs) {
  const NS = 'http://www.w3.org/2000/svg';
  const el = document.createElementNS(NS, tag);
  Object.entries(attrs).forEach(([k,v]) => el.setAttribute(k, v));
  return el;
}

function getArcPath(cx, cy, r, angle) {
  const rad = angle * Math.PI / 180;
  const a1 = rad - (80 * Math.PI / 180);
  const a2 = rad + (80 * Math.PI / 180);
  const x1 = cx + r * Math.cos(a1);
  const y1 = cy + r * Math.sin(a1);
  const x2 = cx + r * Math.cos(a2);
  const y2 = cy + r * Math.sin(a2);
  return `M ${x1} ${y1} A ${r} ${r} 0 0 1 ${x2} ${y2}`;
}

function drawChair(g, cx, cy, r, angle) {
  // Chair circle
  g.appendChild(svgEl('circle', {
    cx, cy, r,
    fill: '#FFFFFF',
    stroke: '#9CA3AF',
    'stroke-width': 1.2
  }));
  // Backrest arc (160 deg centered on angle)
  g.appendChild(svgEl('path', {
    d: getArcPath(cx, cy, r + 3, angle),
    fill: 'none',
    stroke: '#9CA3AF',
    'stroke-width': 1.5
  }));
}

function renderTableLabels(g, cx, cy, id, remaining, total) {
  const text = svgEl('text', { x: cx, y: cy - 5, 'text-anchor': 'middle', 'font-size': '12', 'font-weight': 'bold', fill: '#111827' });
  text.textContent = id;
  g.appendChild(text);
  const sub = svgEl('text', { x: cx, y: cy + 12, 'text-anchor': 'middle', 'font-size': '10', fill: '#6B7280' });
  sub.textContent = `${remaining ?? total}/${total}`;
  g.appendChild(sub);
}

function renderTableSVG(cfg, api, isSelected) {
  const status = api.status || 'FREE';
  const tc = TABLE_COLORS[cfg.id];
  
  // Mesele au acum culoarea lor de identitate de la început (nu doar când sunt ocupate)
  const fill   = isSelected ? '#FFFFFF' : tc.bg;
  const stroke = isSelected ? ACCENT : tc.bar;
  const sw     = isSelected ? 2 : 1.5;

  const g = svgEl('g', {});
  const capacity = cfg.seats;

  if (capacity === 2) {
    // 100x100 Circle table
    g.appendChild(svgEl('circle', { cx: 50, cy: 50, r: 28, fill, stroke, 'stroke-width': sw }));
    drawChair(g, 8, 50, 10, 180);
    drawChair(g, 92, 50, 10, 0);
    renderTableLabels(g, 50, 50, cfg.id, api.seatsRemaining, cfg.seats);
    return { g, w: 100, h: 100 };
  } else if (capacity === 3) {
    // 100x100 Circle table (3 seats)
    g.appendChild(svgEl('circle', { cx: 50, cy: 50, r: 28, fill, stroke, 'stroke-width': sw }));
    const angles = [-90, 30, 150];
    angles.forEach(angle => {
      const rad = angle * Math.PI / 180;
      const cx = 50 + 42 * Math.cos(rad);
      const cy = 50 + 42 * Math.sin(rad);
      drawChair(g, cx, cy, 10, angle);
    });
    renderTableLabels(g, 50, 50, cfg.id, api.seatsRemaining, cfg.seats);
    return { g, w: 100, h: 100 };
  } else if (capacity === 4) {
    // 130x110 Rect table
    g.appendChild(svgEl('rect', { x: 25, y: 20, width: 80, height: 70, rx: 8, fill, stroke, 'stroke-width': sw }));
    drawChair(g, 65, 8, 10, -90);
    drawChair(g, 65, 102, 10, 90);
    drawChair(g, 12, 55, 10, 180);
    drawChair(g, 118, 55, 10, 0);
    renderTableLabels(g, 65, 55, cfg.id, api.seatsRemaining, cfg.seats);
    return { g, w: 130, h: 110 };
  } else if (capacity === 5) {
    // 150x150 Circle table (5 seats)
    g.appendChild(svgEl('circle', { cx: 75, cy: 75, r: 38, fill, stroke, 'stroke-width': sw }));
    const angles = [-90, -18, 54, 126, 198];
    angles.forEach(angle => {
      const rad = angle * Math.PI / 180;
      const cx = 75 + 56 * Math.cos(rad);
      const cy = 75 + 56 * Math.sin(rad);
      drawChair(g, cx, cy, 11, angle);
    });
    renderTableLabels(g, 75, 75, cfg.id, api.seatsRemaining, cfg.seats);
    return { g, w: 150, h: 150 };
  } else if (capacity === 6) {
    // 150x150 Circle table
    g.appendChild(svgEl('circle', { cx: 75, cy: 75, r: 38, fill, stroke, 'stroke-width': sw }));
    for (let i = 0; i < 6; i++) {
        const angle = -90 + i * 60;
        const rad = angle * Math.PI / 180;
        const cx = 75 + 56 * Math.cos(rad);
        const cy = 75 + 56 * Math.sin(rad);
        drawChair(g, cx, cy, 11, angle);
    }
    renderTableLabels(g, 75, 75, cfg.id, api.seatsRemaining, cfg.seats);
    return { g, w: 150, h: 150 };
  }
}

function renderFloorPlan() {
  const wrap = $('floorPlan');
  const NS = 'http://www.w3.org/2000/svg';
  const svg = document.createElementNS(NS, 'svg');
  svg.setAttribute('viewBox', '0 0 880 420');
  svg.setAttribute('fill', 'none');

  svg.innerHTML = `
    <rect x="20" y="20" width="410" height="380" rx="8"
          fill="none" stroke="#111827" stroke-dasharray="6,4" />
    <text x="225" y="50" text-anchor="middle"
          font-size="11" fill="#111827" font-weight="600"
          letter-spacing="0.06em">INDOOR ZONE</text>
    <text x="225" y="66" text-anchor="middle"
          font-size="10" fill="#111827">Main Hall</text>
    <rect x="450" y="20" width="410" height="380" rx="8"
          fill="none" stroke="#111827" stroke-dasharray="6,4" />
    <text x="665" y="50" text-anchor="middle"
          font-size="11" fill="#111827" font-weight="600"
          letter-spacing="0.06em">GARDEN TERRACE</text>
    <text x="665" y="66" text-anchor="middle"
          font-size="10" fill="#111827">Garden and Terrace</text>
  `;

  TABLE_LAYOUT.forEach(cfg => {
    const api = tablesData.find(t => t.id === cfg.id) || {};
    const isSelected = selectedTable === cfg.id;
    const tableData = renderTableSVG(cfg, api, isSelected);
    
    const tableG = tableData.g;
    tableG.setAttribute('class', 'table-group');
    tableG.setAttribute('transform', `translate(${cfg.cx - tableData.w/2}, ${cfg.cy - tableData.h/2})`);
    tableG.onclick = () => selectTable(cfg.id);
    
    svg.appendChild(tableG);
  });

  wrap.innerHTML = '';
  wrap.appendChild(svg);
}

function renderReservations(el) {
  el.innerHTML = `
    <div class="view-header">
      <h2 class="view-title">Reservation List</h2>
      <button class="btn-primary" style="flex:none; width:auto; border-radius:6px; padding:8px 16px; font-weight:500;" id="btnNewRes">+ New Reservation</button>
    </div>
    <table class="data-table">
      <thead>
        <tr>
          <th>#</th>
          <th>Client</th>
          <th>Table</th>
          <th>Location</th>
          <th>Guests</th>
          <th>Date & Time</th>
          <th>Type</th>
        </tr>
      </thead>
      <tbody id="resTableBody"></tbody>
    </table>
    <div id="resEmpty" class="empty-state" style="display:none">No reservations for this day.</div>
  `;
  
  $('btnNewRes').onclick = () => openCreatePanel();
  
  const tbody = $('resTableBody');
  if (!reservations.length) {
    if ($('resEmpty')) $('resEmpty').style.display = 'block';
    return;
  }
  
  reservations.forEach((r, idx) => {
    const dt = new Date(r.dateTime);
    const dtStr = isNaN(dt) ? r.dateTime : `${pad2(dt.getDate())} ${MONTHS[dt.getMonth()].slice(0,3)} ${dt.getFullYear()}, ${pad2(dt.getHours())}:${pad2(dt.getMinutes())}`;
    const tc = TABLE_COLORS[r.masaId] || { bar: '#9CA3AF', bg: '#F3F4F6', text: '#374151' };
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td>${idx + 1}</td>
      <td><strong>${esc(r.clientName)}</strong></td>
      <td><span class="pill" style="background:${tc.bg}; color:${tc.text}; border:1px solid ${tc.bar}">Table ${r.masaId}</span></td>
      <td><span class="pill pill-slate">${r.location}</span></td>
      <td>${r.guests}</td>
      <td>${dtStr}</td>
      <td><span class="pill ${pillClass(r.specific)}">${TYPE_COLORS[r.specific]?.label || r.specific}</span></td>
    `;
    tr.onclick = () => selectTable(r.masaId);
    tbody.appendChild(tr);
  });
}

function renderTables(el) {
  el.innerHTML = `
    <div class="view-header">
      <h2 class="view-title">Table Inventory</h2>
    </div>
    <div class="grid-container">
      ${TABLE_LAYOUT.map(cfg => {
        const api = tablesData.find(t => t.id === cfg.id) || {};
        const isOccupied = api.status === 'OCCUPIED' || api.status === 'PARTIAL';
        const loc = api.location === 'INDOOR' ? 'Indoor' : 'Outdoor';
        return `
          <div class="table-inventory-card">
            <div class="table-card-row">
              <span class="table-card-id">Table ${cfg.id}</span>
              <span class="pill ${cfg.zone === 'INDOOR' ? 'pill-slate' : 'pill-sky'}">${loc}</span>
            </div>
            <div style="color:var(--text-secondary); font-size:14px;">Capacity: ${api.capacity || cfg.seats} seats</div>
            <div>
              <span class="pill ${isOccupied ? 'pill-amber' : 'pill-green'}">${isOccupied ? 'Occupied' : 'Free'}</span>
            </div>
          </div>
        `;
      }).join('')}
    </div>
  `;
}

function renderClients(el) {
  const all = window.allReservations || [];
  const clientsMap = new Map();
  all.forEach(r => {
    if (!clientsMap.has(r.clientName)) {
      clientsMap.set(r.clientName, { name: r.clientName, count: 0, lastDate: null });
    }
    const c = clientsMap.get(r.clientName);
    c.count++;
    const dt = new Date(r.dateTime);
    if (!c.lastDate || dt > c.lastDate) c.lastDate = dt;
  });
  
  const clients = Array.from(clientsMap.values()).sort((a,b) => b.lastDate - a.lastDate);
  
  el.innerHTML = `
    <div class="view-header">
      <h2 class="view-title">Clients</h2>
    </div>
    <div class="client-list">
      ${clients.length ? clients.map(c => {
        const initials = c.name.split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase();
        const lastStr = c.lastDate ? `${pad2(c.lastDate.getDate())} ${MONTHS[c.lastDate.getMonth()]} ${c.lastDate.getFullYear()}` : '—';
        
        // Find most recent table for this client
        const clientRes = all.filter(r => r.clientName === c.name).sort((a,b) => new Date(b.dateTime) - new Date(a.dateTime));
        const lastTableId = clientRes[0]?.masaId;
        const tc = TABLE_COLORS[lastTableId] || stringToColor(c.name);

        return `
          <div class="client-row">
            <div class="res-avatar" style="background:${tc.bg}; color:${tc.text}; border:1px solid ${tc.bar || 'transparent'}">${initials}</div>
            <div class="client-row-content">
              <div style="font-weight:600;">${esc(c.name)}</div>
              <div class="client-meta">${c.count} reservations &middot; Last: ${lastStr} (Table ${lastTableId})</div>
            </div>
            <div class="client-chevron">&rsaquo;</div>
          </div>
        `;
      }).join('') : '<div class="empty-state">No clients registered.</div>'}
    </div>
  `;
}

function selectTable(id) {
  if (selectedTable === id && $('detailPanel').classList.contains('open')) { closePanel(); return; }
  selectedTable = id;
  if (currentView === 'dashboard') renderFloorPlan();

  const api  = tablesData.find(t => t.id == id) || {};
  const cfg  = TABLE_LAYOUT.find(t => t.id == id);
  const loc  = cfg.zone === 'INDOOR' ? 'Indoor' : 'Outdoor';
  const res  = reservations.find(r => r.masaId === id);

  const body = $('detailBody');
  const head = document.querySelector('.detail-head');
  const tc = TABLE_COLORS[id] || { bg: '#FFFFFF', bar: '#E5E7EB' };
  
  if (head) {
    head.style.background = tc.bg;
    head.style.borderBottom = `1px solid ${tc.bar}`;
  }
  
  body.innerHTML = '';

  if (res) {
    const specColors = TYPE_COLORS[res.specific] || {};
    const remaining  = api.seatsRemaining ?? '—';
    const remClass   = remaining <= 0 ? 'pill-red' : remaining <= 2 ? 'pill-amber' : 'pill-green';

    const dt = new Date(res.dateTime);
    const dtStr = isNaN(dt) ? res.dateTime :
      `${pad2(dt.getDate())} ${MONTHS[dt.getMonth()].slice(0,3)} ${dt.getFullYear()}, ${pad2(dt.getHours())}:${pad2(dt.getMinutes())}`;

    body.innerHTML = `
      ${propRow('Client', `<strong>${esc(res.clientName)}</strong>`)}
      ${propRow('Guests', res.guests)}
      ${propRow('Date', dtStr)}
      ${propRow('Type', `<span class="pill ${pillClass(res.specific)}">${specColors.label || res.specific}</span>`)}
      ${propRow('Location', `<span class="pill pill-slate">${res.location}</span>`)}
      ${propRow('Seats remaining', `<span class="pill ${remClass}">${remaining}</span>`)}
      <div class="detail-note">
        <label>Notes</label>
        <textarea placeholder="No notes...">${res.specific === 'BIRTHDAY' ? 'Birthday \u2014 surprise cake, quiet spot' : ''}</textarea>
      </div>
      <div style="margin-top:16px; text-align:center;">
         <a href="#" style="color:#EF4444; text-decoration:none; font-size:14px; font-weight:600;" id="btnDeleteRes">Cancel this reservation</a>
      </div>`;
      
    $('btnDeleteRes').onclick = (e) => {
        e.preventDefault();
        deleteReservation(res.masaId, res.dateTime);
    };
    
    const confirmBtn = $('btnConfirm');
    confirmBtn.textContent = 'Confirm';
    confirmBtn.onclick = closePanel;
  } else {
    body.innerHTML = `
      <div class="empty-state" style="padding-top:20px;">Table available.</div>
      <div style="display:flex; justify-content:center; margin-top:16px;">
        <button class="btn-primary" style="flex:none; padding:10px 24px;" id="btnAddResPlan">+ Add Reservation</button>
      </div>
    `;
    $('btnAddResPlan').onclick = () => openCreatePanel(id);
    const confirmBtn = $('btnConfirm');
    confirmBtn.textContent = 'Confirm';
    confirmBtn.onclick = closePanel;
  }
  
  $('detailPanel').classList.add('open');
}

function openCreatePanel(tableId = null) {
  pendingTableId = tableId;
  $('detailTitle').textContent = tableId ? `New Reservation · Table ${tableId}` : 'New Reservation';
  $('detailCaption').textContent = 'Enter client details';

  const head = document.querySelector('.detail-head');
  if (tableId && head) {
    const tc = TABLE_COLORS[tableId];
    head.style.background = tc.bg;
    head.style.borderBottom = `1px solid ${tc.bar}`;
  } else if (head) {
    head.style.background = '#FFFFFF';
    head.style.borderBottom = '1px solid var(--border)';
  }
  
  const now = new Date();
  const dateVal = `${now.getFullYear()}-${pad2(now.getMonth()+1)}-${pad2(now.getDate())}T${pad2(now.getHours())}:00`;
 
  const body = $('detailBody');
  body.innerHTML = `
    <div style="display:flex; flex-direction:column; gap:16px;">
      <div class="detail-note" style="margin-top:0">
        <label>Client Name</label>
        <input type="text" id="inNume" class="detail-input" placeholder="ex: John Smith" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:6px; outline:none; font-family:var(--font);">
      </div>
      <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
        <div class="detail-note" style="margin-top:0">
          <label>Guests</label>
          <input type="number" id="inPers" class="detail-input" value="2" min="1" max="10" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:6px; outline:none; font-family:var(--font);">
        </div>
        <div class="detail-note" style="margin-top:0">
          <label>Location</label>
          <select id="inLoc" class="detail-input" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:6px; outline:none; font-family:var(--font); height:38px;">
            <option value="INDOOR" ${tableId && TABLE_LAYOUT.find(t=>t.id==tableId).zone==='INDOOR' ? 'selected' : ''}>Indoor</option>
            <option value="OUTDOOR" ${tableId && TABLE_LAYOUT.find(t=>t.id==tableId).zone==='OUTDOOR' ? 'selected' : ''}>Outdoor</option>
          </select>
        </div>
      </div>
      <div class="detail-note" style="margin-top:0">
        <label>Date & Time</label>
        <input type="datetime-local" id="inData" value="${dateVal}" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:6px; outline:none; font-family:var(--font);">
      </div>
      <div class="detail-note" style="margin-top:0">
        <label>Reservation Details</label>
        <select id="inSpec" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:6px; outline:none; font-family:var(--font); height:38px;">
          <option value="FAMILY">Family</option>
          <option value="FRIENDS">Friends</option>
          <option value="MEETING">Meeting</option>
          <option value="BIRTHDAY">Birthday</option>
        </select>
      </div>
    </div>
  `;
  
  const confirmBtn = $('btnConfirm');
  confirmBtn.textContent = 'Confirm Reservation';
  confirmBtn.onclick = submitReservation;
  
  $('detailPanel').classList.add('open');
}

async function submitReservation() {
  const data = {
    clientName: $('inNume').value,
    guests: parseInt($('inPers').value),
    dateTime: $('inData').value + ':00',
    location: $('inLoc').value,
    specific: $('inSpec').value,
    tableId: pendingTableId
  };
  
  if (!data.clientName) { alert('Please enter the client name.'); return; }

  try {
    const res = await fetch(`${API}/api/reservation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    const result = await res.json();
    if (result.success) {
      closePanel();
      loadData();
    } else {
      alert(result.message || 'Error creating reservation.');
    }
  } catch (e) {
    alert('Error connecting to Server.');
  }
}

function closePanel() {
  selectedTable = null;
  pendingTableId = null;
  $('detailPanel').classList.remove('open');
  const head = document.querySelector('.detail-head');
  if (head) {
    head.style.background = '#FFFFFF';
    head.style.borderBottom = '1px solid var(--border)';
  }
  if (currentView === 'dashboard') renderFloorPlan();
}

async function deleteReservation(masaId, dateTime) {
  if (!confirm('Are you sure you want to delete this reservation?')) return;
  try {
    const res = await fetch(`${API}/api/reservation?tableId=${masaId}&datetime=${encodeURIComponent(dateTime)}`, {
      method: 'DELETE'
    });
    const result = await res.json();
    if (result.success) {
      closePanel();
      loadData();
    } else {
      alert(result.message || 'Error deleting reservation.');
    }
  } catch {
    alert('Error connecting to Server.');
  }
}

function propRow(label, value) {
  return `<div class="prop-row"><span class="prop-label">${label}</span><span class="prop-value">${value}</span></div>`;
}

function pillClass(specific) {
  return { BIRTHDAY:'pill-amber', FAMILY:'pill-green', FRIENDS:'pill-blue', MEETING:'pill-purple' }[specific] || 'pill-slate';
}

function stringToColor(str) {
  let hash = 0;
  for (let i = 0; i < str.length; i++) hash = str.charCodeAt(i) + ((hash << 5) - hash);
  const h = Math.abs(hash) % 360;
  return { bg: `hsl(${h}, 40%, 95%)`, text: `hsl(${h}, 60%, 30%)` };
}

function renderResListSidebar() {
  const el = $('resList');
  $('resCount').textContent = reservations.length;

  if (!reservations.length) {
    el.innerHTML = '<div class="empty-state">No reservations for this day.</div>';
    return;
  }
  el.innerHTML = '';
  reservations.forEach(r => {
    const tc = TABLE_COLORS[r.masaId] || { bar: '#9CA3AF', bg: '#F3F4F6', text: '#374151' };
    const initials = r.clientName.split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase();
    const dt = new Date(r.dateTime);
    const timeStr = isNaN(dt) ? '' : `${pad2(dt.getHours())}:${pad2(dt.getMinutes())}`;

    const card = document.createElement('div');
    card.className = 'res-card';
    card.innerHTML = `
      <div class="res-bar" style="background:${tc.bar}"></div>
      <div class="res-avatar" style="background:${tc.bg};color:${tc.text}; border:1px solid ${tc.bar}">${initials}</div>
      <div class="res-info">
        <div class="res-name">${esc(r.clientName)}</div>
        <div class="res-details">Table ${r.masaId} &middot; ${timeStr}</div>
      </div>
      <div class="res-badge" style="background:${tc.bg}; color:${tc.text}; border:1px solid ${tc.bar}">${r.guests} guests</div>`;
    card.style.cursor = 'pointer';
    card.onclick = () => {
      if (currentView !== 'dashboard') switchView('dashboard');
      selectTable(r.masaId);
    };
    el.appendChild(card);
  });
}

function pad2(n) { return String(n).padStart(2, '0'); }
function esc(s) { return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
function fmtISO(d, h, m) {
  return `${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}T${pad2(h)}:${pad2(m)}:00`;
}
